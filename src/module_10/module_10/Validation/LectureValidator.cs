...
namespace RestApi.Validation
{
    internal class LectureValidator : AbstractValidator<LectureValid>
    {
        public LectureValidator()
        {
            ...
            RuleFor(x => x.HomeworkId).Must(x => x == null || x > 0).WithMessage("Homework id must be empty or greater than 0.");
            RuleFor(x => x.LecturerId).GreaterThan(0).WithMessage("Lecturer id must be greater than 0.");
        }
    }
}
